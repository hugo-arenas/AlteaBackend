...
public class PatologiaRepositoryImp implements PatologiaRepository {
    ...
    public List<Patologia> getAllPatologia() {
        try (Connection conn = sql2o.open()) {
            return conn
                    .createQuery(
                            "ID AS ID, Nombre AS Nombre, Descripcion AS Descripcion, Recomendacion AS Recomendacion")
                    .executeAndFetch(Patologia.class);
    ...
    public Patologia createPatologia(Patologia Patologia) {
        try (Connection conn = sql2o.open()) {
            Patologia v1 = conn.createQuery("select * from Patologia where Nombre=:Nombre")
                    .addParameter("Nombre", Patologia.getNombre()).executeAndFetchFirst(Patologia.class);
            if (v1 == null) {
                long insertedId = countPatologia() + 1;
                conn.createQuery("insert into Patologia (ID, Nombre, Descripcion, Recomendacion)"
                        ...
                        .addParameter("id", insertedId).addParameter("PatologiaNombre", Patologia.getNombre())
                        .addParameter("PatologiaDescripcion", Patologia.getDescripcion())
                        .addParameter("PatologiaRecomendacion", Patologia.getRecomendacion()).executeUpdate().getKey();
    ...
    public boolean deletePatologia(long id) {
        try (Connection conn = sql2o.open()) {
            conn.createQuery("DELETE FROM Patologia WHERE ID = :id").addParameter("id", id).executeUpdate();
            return true;
        } catch (Exception e) {
            System.out.println(e.getMessage());
            return false;
    ...
    public Patologia getPatologia(long id) {
        ...
        try (Connection con = sql2o.open()) {
            return con.createQuery(sql).addParameter("id", id).executeAndFetchFirst(Patologia.class);
        } catch (Exception e) {
            System.out.println(e.getMessage());
            return null;
        }
    ...
    public boolean updatePatologia(Patologia nuevoPatologia) {
        ...
        try (Connection con = sql2o.open()) {
            con.createQuery(updateSql).addParameter("Recomendacion", nuevoPatologia.getRecomendacion())
                    .addParameter("Descripcion", nuevoPatologia.getDescripcion())
                    .addParameter("Nombre", nuevoPatologia.getNombre()).addParameter("id", nuevoPatologia.getId())
                    .executeUpdate();
            return true;
        } catch (Exception e) {
            System.out.println(e.getMessage());
